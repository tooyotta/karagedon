...
class BatchedRemoveStatusService < BaseService
  ...
  def call(statuses, **options)
    ...
    # Notifications are associated polymorphically and therefore not
    # in the foreign key system, for this reason, we have to delete them
    # first. They don't have any side-effects, so delete can be used.
    Notification.joins(:status).where(status: statuses_and_reblogs).delete_all
    Notification.joins(favourite: :status).where(favourites: { status: statuses_and_reblogs }).delete_all
    Notification.joins(mention: :status).where(mentions: { status: statuses_and_reblogs }).delete_all
